import {
  ChangeDetectionStrategy,
  Component,
  EventEmitter,
  Input,
  OnChanges,
  Output,
  SimpleChanges
} from '@angular/core';
import {FlatNode, MenuNode} from "./home-navbar.model";
import {FlatTreeControl} from "@angular/cdk/tree";
import {MatTreeFlatDataSource, MatTreeFlattener} from "@angular/material/tree";
import {Menu} from "@angular/cdk/menu";

@Component({
  selector: 'home-navbar',
  template: `
    <header>
      <mat-toolbar color="primary">
        <img
          src="data:image/png;base64,"
          alt="logo" class="home-navbar__logo">
      </mat-toolbar>
    </header>
    <mat-tree [dataSource]="dataSource" [treeControl]="treeControl">
      <mat-tree-node [className]="navItemClass(node)" *matTreeNodeDef="let node;">
        <div class="primary-background home-navbar__node" (click)="onClick(node)">
          <mat-icon disabled=""></mat-icon>
          <span style="color: white">
              {{node.name}}
              </span>
          <span class="spacer"></span>
        </div>
        <!-- use a disabled button to provide padding for tree leaf -->
      </mat-tree-node>

      <!-- This is the tree node template for expandable nodes -->
      <mat-tree-node [className]="navItemClass(node)" *matTreeNodeDef="let node;when: hasChild">
        <div class="primary-background home-navbar__node" (click)="onClick(node)">
          <mat-icon disabled=""></mat-icon>
          <span style="color: white">
              {{node.name}}
              </span>
          <span class="spacer"></span>
          <mat-icon style="padding: 0px 5px; color:white">
            {{isParentActive(node) ? 'expand_more' : 'chevron_right'}}
          </mat-icon>
          <mat-icon disabled=""></mat-icon>
        </div>
      </mat-tree-node>

      <mat-tree-node [className]="navItemClass(node)" *matTreeNodeDef="let node;when: isChild">
        <div class="primary-background home-navbar__node" (click)="onClick(node)">
          <mat-icon disabled=""></mat-icon>
          <mat-icon disabled=""></mat-icon>
          <span style="color: white">
              {{node.name}}
              </span>
        </div>
      </mat-tree-node>
    </mat-tree>
  `,
  styleUrls: ['./home-navbar.component.scss'],
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class HomeNavbarComponent implements OnChanges {
  @Input()
  activeRoute: string = '';

  @Output()
  selectRoute = new EventEmitter<String>;

  activeNode?: FlatNode;
  activeParent?: FlatNode;

  data: MenuNode[] = [
    {
      name: 'Admin',
      route: '/admin'
    },
    {
      name: 'Member',
      route: '/member',
    },
    {
      name: 'Alumni',
      route: '/alumni',
      children: [
        { name: 'Alumni Application', route: '/alumni/application'}
      ]
    },
    {
      name: 'Activity',
      route: '/activity'
    },
    {
      name: 'Notification',
      route: '/notification',
    }
  ];

  private _transformer = (node: MenuNode, level: number) => {
    return {
      expandable: !!node.children && node.children.length > 0,
      name: node.name,
      level: level,
      route: node.route,
    }
  }
  treeControl = new FlatTreeControl<FlatNode>(
    node => node.level,
    node => node.expandable,
  )

  treeFlattener = new MatTreeFlattener(
    this._transformer,
    node => node.level,
    node => node.expandable,
    node => node.children,
  )

  dataSource = new MatTreeFlatDataSource(this.treeControl, this.treeFlattener)
  hasChild = (_: number, node: FlatNode) => node.expandable;

  isChild = (_: number, node: FlatNode) => node.level > 0;

  ngOnChanges(changes: SimpleChanges): void {
    this.dataSource.data = this.data;
    this.activeParent = this.treeControl.dataNodes.find(this.isParentActive)
    this.activeNode = this.treeControl.dataNodes.find(this.isNodeActive);


    if (this.activeParent) {
      this.treeControl.expand(this.activeParent);
    }
  }

  isNodeActive = (node: FlatNode) => this.activeRoute === node.route;

  isParentActive = (node: FlatNode) => node.expandable && this.activeRoute.includes(node.route);

  navItemClass = (node: FlatNode) => this.isNodeActive(node) ?
    'home-navbar__item__active' : this.isParentActive(node) ?
      'home-navbar__parent__active' : 'home-navbar__item';

  onClick(node: FlatNode) {
    this.selectRoute.emit(node.route);
  }
}
